...
#   backend = "s3"
#   #   config  = merge(var.remote_state.kms.seeun-id.apne2)
#   config = merge(var.remote_state.kms.seeun-id.apne2, { "assume_role" = { "role_arn" = var.assume_role_arn } })
# }

# data "terraform_remote_state" "secretsmanager" {
#   backend = "s3"
#   #   config  = merge(var.remote_state.kms.seeun-id.apne2)
#   config = merge(var.remote_state.secretsmanager.seeun-id.apne2, { "assume_role" = { "role_arn" = var.assume_role_arn } })
# }
...
#   backend = "s3"
#   #   config  = merge(var.remote_state.codedeploy.seeun-id.apne2)
#   config = merge(var.remote_state.codedeploy.seeun-id.apne2, { "assume_role" = { "role_arn" = var.assume_role_arn } })
# }
