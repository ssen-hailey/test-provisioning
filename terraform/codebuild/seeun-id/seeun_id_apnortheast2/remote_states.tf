data "terraform_remote_state" "vpc" {
  ...
  config = merge(var.remote_state.vpc.sed_apnortheast2, { "assume_role" = { "role_arn" = var.assume_role_arn } })
}
...
data "terraform_remote_state" "security_group" {
  ...
  config = merge(var.remote_state.security_group.seeun-id.sedapne2, { "assume_role" = { "role_arn" = var.assume_role_arn } })
}
